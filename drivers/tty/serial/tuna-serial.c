...
int omap_serial_wake(int port_index)
{
	...
	pm_wakeup_event(&ui[port_index]->pdev->dev, 0);
...
int omap_serial_enable(int port_index)
{
	...
	pm_runtime_get_sync(&ui[port_index]->pdev->dev);
...
int omap_serial_disable(int port_index)
{
	...
	pm_runtime_put_sync_suspend(&ui[port_index]->pdev->dev);
			...
			struct ktermios *old)
{
	...
	unsigned char efr __maybe_unused = 0;
	unsigned long flags = 0;
...
static int omap_serial_runtime_suspend(struct device *dev)
{
	...
	if (device_may_wakeup(dev))
		up->enable_wakeup(dev, true);
	else
		up->enable_wakeup(dev, false);
done:
	return 0;
}
