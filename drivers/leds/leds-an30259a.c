...
struct an30259a_data {
	...
	struct led_classdev	led_r;
	struct led_classdev	led_g;
	struct led_classdev	led_b;
	u32			shadow_val;
...
static void an30259a_brightness_set(struct an30259a_data *data,
				     enum an30259a_led led,
				     u8 brt_val)
{
	...
	switch (led) {
	case LED_R:
		val = (brt_val & LED_R_MASK) >> LED_R_SHIFT;
		mask = (LED_R_MASK >> LED_R_SHIFT);
		break;
	case LED_G:
		val = (brt_val & LED_G_MASK) >> LED_G_SHIFT;
		mask = (LED_G_MASK >> LED_G_SHIFT);
		break;
	case LED_B:
		val = brt_val & LED_B_MASK;
		mask = LED_B_MASK;
		break;
	default:
		return;
	...
	data->shadow_val &= mask;
	data->shadow_val |= val;

	if (data->shadow_val)
		leds_on(client, led, true, false, brt_val);
	else
		leds_on(client, led, false, false, 0);

	leds_i2c_write_all(client);
}

static void an30259a_brightness_set_r(struct led_classdev *led_cdev,
				     enum led_brightness brt_val)
{
	struct an30259a_data *data =
	    container_of(led_cdev, struct an30259a_data, led_r);

	an30259a_brightness_set(data, LED_R,
		min((unsigned)led_cdev->max_brightness, (unsigned)brt_val));
}

static void an30259a_brightness_set_g(struct led_classdev *led_cdev,
				     enum led_brightness brt_val)
{
	struct an30259a_data *data =
	    container_of(led_cdev, struct an30259a_data, led_g);

	an30259a_brightness_set(data, LED_G,
		min((unsigned)led_cdev->max_brightness, (unsigned)brt_val));
}

static void an30259a_brightness_set_b(struct led_classdev *led_cdev,
				     enum led_brightness brt_val)
{
	struct an30259a_data *data =
	    container_of(led_cdev, struct an30259a_data, led_b);

   	an30259a_brightness_set(data, LED_B,
		min((unsigned)led_cdev->max_brightness, (unsigned)brt_val));
}
	...
	const struct i2c_device_id *id)
{
	...
	data->client = client;
	data->shadow_val = 0;
	data->led_r.name = AN30259A_NAME ":red";
	data->led_r.brightness_set = an30259a_brightness_set_r;
	data->led_r.max_brightness = MAX_BRIGHTNESS;
	data->led_g.name = AN30259A_NAME ":green";
	data->led_g.brightness_set = an30259a_brightness_set_g;
	data->led_g.max_brightness = MAX_BRIGHTNESS;
	data->led_b.name = AN30259A_NAME ":blue";
	data->led_b.brightness_set = an30259a_brightness_set_b;
	data->led_b.max_brightness = MAX_BRIGHTNESS;

	i2c_set_clientdata(client, data);

	/* initialize LED */
	err = an30259a_initialize(client);
	if (err < 0) {
		dev_err(&client->adapter->dev, "failure on initialization\n");
		goto exit;
	}

	err = led_classdev_register(&client->dev, &data->led_r);
	if (err < 0) {
		dev_err(&client->dev, "Register led class failed: %d\n", err);
		goto exit;
	}
	err = led_classdev_register(&client->dev, &data->led_g);
	if (err < 0) {
		dev_err(&client->dev, "Register led class failed: %d\n", err);
		goto err_red;
	}
	err = led_classdev_register(&client->dev, &data->led_b);
	if (err < 0) {
		dev_err(&client->dev, "Register led class failed: %d\n", err);
	}
	goto err_green;

	/*err = device_create_file(data->led_dev.dev, &dev_attr_mode);
	if (err < 0) {
	...
	led_classdev_unregister(&data->led_b);
err_green:
	led_classdev_unregister(&data->led_g);
err_red:
	led_classdev_unregister(&data->led_r);
exit:
	return err;
}
...
static int an30259a_remove(struct i2c_client *client)
{
	...
	/*device_remove_file(data->led_dev.dev, &dev_attr_mode);*/
	dev_dbg(&client->adapter->dev, "%s\n", __func__);
	led_classdev_unregister(&data->led_r);
	led_classdev_unregister(&data->led_g);
	led_classdev_unregister(&data->led_b);
