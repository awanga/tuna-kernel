...
{
	if (to_gpios) {
		gpio_direction_output(pdata->gpio_resetb, 0);
		gpio_direction_input(pdata->gpio_resetb);
		...
		gpio_direction_output(pdata->gpio_resetb, 1);
		gpio_direction_input(pdata->gpio_resetb);
...
static struct mms_ts_platform_data *mms144_parse_dt(struct device *dev)
{
	...
	pdata->gpio_resetb = of_get_named_gpio(np, "gpio-reset", 0);
	if (!gpio_is_valid(pdata->gpio_resetb)) {
		dev_err(dev, "failed to get reset gpio\n");
		return NULL;
	};

	pdata->gpio_vdd_en = of_get_named_gpio(np, "gpio-enable", 0);
	if (!gpio_is_valid(pdata->gpio_vdd_en)) {
		dev_err(dev, "failed to get reset gpio\n");
		return NULL;
	};

	pdata->gpio_scl = of_get_named_gpio(np, "gpio-scl", 0);
	if (!gpio_is_valid(pdata->gpio_scl)) {
		dev_err(dev, "failed to get scl gpio\n");
		return NULL;
	};

	pdata->gpio_sda = of_get_named_gpio(np, "gpio-sda", 0);
