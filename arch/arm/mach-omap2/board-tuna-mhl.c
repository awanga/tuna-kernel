...
struct sii9234_data {
	struct tuna_sii9234_platform_data	*pdata;
...
static int sii9234_detection_callback(struct notifier_block *nb)
{
...
static void sii9234_cancel_callback(struct notifier_block *nb)
{
	struct sii9234_data *sii9234 = container_of(nb, struct sii9234_data,
						usb_nb);
...
static int sii9234_callback(struct notifier_block *nb, unsigned long action, void *data)
{
	if (action == USB_DEVICE_ADD) {
		if (sii9234_detection_callback(nb) == OTG_ID_HANDLED)
			return NOTIFY_STOP;
	}
	else if (action == USB_DEVICE_REMOVE)
		sii9234_cancel_callback(nb);
...
static void sii9234_retry_detection(struct work_struct *work)
{
	...
	if (sii9234_detection_callback(&sii9234->usb_nb) == OTG_ID_UNHANDLED)
			...
			const struct i2c_device_id *id)
{
	...
	sii9234->usb_nb.notifier_call = sii9234_callback;
	sii9234->usb_nb.priority = sii9234->pdata->prio;
	usb_register_notify(&sii9234->usb_nb);
...
#if 0
static int mhl_probe(struct usb_interface *iface,
...
static void mhl_disconnect(struct usb_interface *iface)
{
	struct sii9234_data *sii9234 = usb_get_intfdata(iface);
...
static struct usb_driver mhl_driver = {
	.name		= "sii9234_mhl",
	.probe		= mhl_probe,
	.disconnect	= mhl_disconnect,
	.id_table	= mhl_table,
};
#endif
...
static int sii9234_tpi_i2c_probe(struct i2c_client *client,
			const struct i2c_device_id *id)
{
	struct tuna_sii9234_platform_data *pdata = client->dev.platform_data;
