...
/* from omap_hwmod.c */
extern struct device_node *of_dev_hwmod_lookup(struct device_node *np,
				  ...
				  struct omap_device_pad *pads, int num_pads)
{
	...
	if (np)
		dbg->debug_port_base = of_iomap(np, 0);
	...
	pdev = omap_device_build("fiq_debugger", id, oh, dbg, sizeof(*dbg));
	if (IS_ERR(pdev)) {
